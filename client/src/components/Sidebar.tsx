...
interface SidebarProps {
    ...
    connectedUsers: ConnectedUserEntry[];
...
export default function Sidebar({
    ...
    const [exampleRuleIds, setExampleRuleIds] = useState<Set<string>>(
        new Set(),
    );
        ...
        <aside>
            ...
            {selectedType === "Example" && (
                <div className="example-rules-field">
                    ...
                    {rules.length === 0 ? (
                        <p className="example-rules-empty">
                            Add at least one rule first, then choose it here.
                        </p>
                    ) : (
                        <ul
                            className="example-rules-list"
                            role="group"
                            aria-label="Rules this example illustrates"
                        >
                            {rules.map((r) => {
                                    ...
                                    <li key={r.id}>
                                        <label className="example-rule-option">
                                            <input
                                                ...
                                                checked={checked}
                                                onChange={() =>
                                                    toggleExampleRule(r.id)
                                                }
                                            />
                                            <span className="example-rule-id">
                                                #{num}
                                            </span>
                                            <span className="example-rule-preview">
                                                {r.content}
                                            </span>
                                        </label>
                                    </li>
                ...
                <ul className="user-list">
                    {connectedUsers.length === 0 ? (
                        ...
                        connectedUsers.map((u) => {
                            ...
                            const label = u.displayName || "Unnamed";
                            return (
                                <li
                                    key={u.socketId}
