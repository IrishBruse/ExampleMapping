        ...
        <main id="board" ref={boardRef}>
            {totalVisible === 0 ? (
                ...
                <>
                    {stories.length > 0 && (
                        <section
                            className="board-section board-section--story"
                            aria-label="Stories"
                        >
                            ...
                            {rules.length === 0 ? (
                                <div className="board-rule-rows board-rule-rows--examples-only">
                                    <p className="board-column-empty board-column-empty--block">
                                        No rules yet — add a rule, then link
                                        examples to it.
                                    </p>
                                    {examples.length > 0 && (
                                        <div className="board-section-cards">
                                ...
                                <div className="board-rule-rows">
                                    {rules.map((rule) => {
                                        const rowExamples =
                                            examplesLinkedToRule(rule.id);
                                        return (
                                            <div
                                                key={rule.id}
                                                className="board-rule-row"
                                                aria-label={`Rule ${rule.id}`}
                                            >
                                                <div className="board-rule-row__rule">
                                                    {renderCard(rule)}
                                                </div>
                    ...
                    {questions.length > 0 && (
                        <section
                            className="board-section board-section--question"
