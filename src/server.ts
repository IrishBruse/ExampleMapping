...
/**
 * Auto-generate `context_files/AGENTS.md` with note-type templates and an
 * index of every note currently on disk.  Called at startup and after any
 * note add / edit / delete so that external agents always see fresh docs.
 */
...
> This file documents the note templates and current state for AI agents.

## Note Types

This project uses four sticky-note types: **Story**, **Rule**, **Example**, **Question**.

Every note is saved as a Markdown file under \`context_files/<username>/<Type>_<N>.md\`
...
The \`Examples\` field lists Example note IDs that illustrate this rule.
...
The \`Rules\` field is **required** — at least one Rule ID must be linked.
...
`;
    ...
    try {
        ...
        console.warn("[notes] could not write AGENTS.md:", e);
    }
}
